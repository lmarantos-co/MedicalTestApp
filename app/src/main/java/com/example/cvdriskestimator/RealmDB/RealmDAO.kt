...
class RealmDAO {
    ...
    fun fetchTestData(patientId : String , testName : String) : MutableLiveData<Test>
    {
        when (testName)
        {
            ...
            "OPQOL Test" ->
            {
                var testList = realm.where(Test::class.java).isNotNull("patientOPQOLQ1").equalTo("testName" , testName).equalTo("patientId" , patientId).findAll()
                if (testList.size > 0)
                    testdata.value = testList.get(testList.size -1)!!
            }
            "GAS" ->
            {
                var testList = realm.where(Test::class.java).isNotNull("patientGASQ1").equalTo("testName" , testName).equalTo("patientId" , patientId).findAll()
                if (testList.size > 0)
                    testdata.value = testList.get(testList.size -1)!!
            }
            "SIDAS" ->
