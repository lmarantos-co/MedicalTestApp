...
class CheckHAMDPatientViewModel : ViewModel() {
    ...
    fun fetchHistoryTest(patientId : String, testDate : Date) : Test
    {
        ...
        realm.executeTransaction {
            ...
            tests = realm.where(Test::class.java).equalTo("patientId" , patientId).between("testDate" , dummyTestDate.time , testDate).equalTo("testName" , "Hammilton Depression").findAll()
        }
